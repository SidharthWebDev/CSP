...
#Spawn Enemy
def spawn_enemy():
    if not game_running or Super_Boss_Spawned:
        return
    weights = []
...
def update_Super_Boss():
    ...
    draw_stats()
    wn.update()
    if game_running and SuperBoss.active:
...
def show_instruction_screen(x=None, y=None):
    writer.clear()
    writer.goto(0, 200)
    writer.write("HOW TO PLAY(READ CAREFULLY)", align="center", font=("Arial", 28, "bold"))
    writer.goto(0, 175)
    ...
    writer.goto(0, 75)
    writer.write("Collect power-ups to help you survive.", align="center", font=("Arial", 20, "normal"))
    writer.goto(0, 50)
    writer.write("Close game window when you're done.", align="center", font=("Arial", 20, "normal"))
    writer.goto(0, 25)
